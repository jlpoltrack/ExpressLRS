...
if platform in ['espressif8266']:
    ...
    # The platform uploads firmware.elf, which lacks the options/hardware layout appended to
    # firmware.bin and leaves the RX with no pin map (it crashes before USB starts), so load the
    # UF2 rebuilt from the configured .bin instead. The load offset and target MCU are baked into
    # the UF2, so picotool rejects an image built for the wrong chip.
...
if platform in ['raspberrypi']:
    ...
    env.AddPostAction("$BUILD_DIR/${PROGNAME}.bin", rp2350_uf2.convertToUF2)
